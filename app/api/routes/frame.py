...
def get_axis_data(camera: Camera = Depends(get_camera), 
...
def set_exposure_time(camera: Camera = Depends(get_camera), 
                      time: float = 200000):
...
@router.get("/frame")
async def get_8_bits_img(type: str = "8_bits"):
    """将视频流作为流响应返回。"""
    data = generate_frames(type)
